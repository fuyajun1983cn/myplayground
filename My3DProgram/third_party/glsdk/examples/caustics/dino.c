...
static GLfloat eye[][2] = { {8.75f, 15}, {9, 14.7f}, {9.6f, 14.7f}, {10.1f, 15},
{9.6f, 15.25f}, {9, 15.25f} };
static GLfloat skinColor[] = {0.1f, 1.0f, 0.1f, 1.0f}, eyeColor[] = {1.0f, 0.2f, 0.2f, 1.0f};
/* *INDENT-ON* */

static void extrudeSolidFromPolygon(GLfloat data[][2], unsigned int dataSize,
									GLdouble thickness, GLuint side, GLuint edge, GLuint whole)
{
	...
	for (i = 0; i <= count; i++)
	{
		...
		glNormal3f((float)(dx / len), (float)(dy / len), 0.0f);
	}
	glEnd();
	glEndList();
	...
	glCallList(side);
	glPushMatrix();
	glTranslatef(0.0f, 0.0f, (float)thickness);
	glFrontFace(GL_CCW);
...
int makeDinosaur(void)
{
	...
	glCallList(LEG_WHOLE);
	glTranslatef(0.0, 0.0, bodyWidth / 2 - 0.1f);
