...
** 概述
   ...
   用于选择特定的认证机制（方法）。Authenticator可以本身支持很多认证方
...
** EAP复用模型
   ...
   打个比方，EAP Code相关IP地址，而Type则相当于端口号，根据Type信息，
   将EAP消息传递到对应的方法中。
** EAP Packet Format
   格式如下：
...
***** Identity
      ...
      Identity或认证失败的情况下，允许至少进行3次重试机会。通常
...
***** Notification
      可的的一种EAP Request/Response类型，Authenticator通过该消息类型
      来向Peer传递一些可显示的消息。
...
*** Peer状态机与EAP方法之间的接口
    ...
    3. 接下来，方法需要根据如下规则更新methodState以及decision:
       ...
       - methodState=DONE
         ...
         decision=FAIL，否则设置decision为UNCOND_SUCC。如果我们不知道服
         务器是否允许我们访问，但是我们自己想访问，则设置decision为COND_SUCC。
    4. 最后，必须设置allowNotifications
       如果新的methodState的值为CONT或MAY_CONT，且方法规范中并不禁止使
...
**** Long-Term (Maintained between Packets)
     ...
     - lastId (integer)
       上一个请求的EAP标识值，在SEND_RESPONSE状态中设置。
     - lastRespData (EAP packet)
       在SEND_RESPONSE状态中设置，存储上次从Peer中发送的EAP数据包。
...
**** Long-Term (Maintained between Packets)
     ...
     - currentId (integer)
       0 ~ 255或NONE。通常中PROPOSE_METHOD状态中更新。表明当前EAP请求的
       标识值。
     - methodState (enumeration)
       方法状态
     - retransCount (integer)
       当前重传的次数。在SEND_REQUEST状态中重置，在RETRANSMIT状态中更新。
...
**** Short-Term (Not Maintained between Packets)
     ...
     - ignore (boolean)
       在METHOD状态中设置，显示当前方法是否决定要丢弃当前的数据包。
     - decision (enumeration)
       在SELECT_ACTION状态中设置，临时地存储策略决定成功，失败还是继续。
